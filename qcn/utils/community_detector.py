...
def community_analizer(ax, fig, keep_plot, network_type, num_nodes):
    ...
    csv_filepath = DATA_DIR / "output.csv"
    logger.debug("%s", csv_filepath)
    network = pd.read_csv(csv_filepath)
    
    """
    csv_filepath_200 = DATA_DIR / "output_200.csv"
    network_200 = pd.read_csv(csv_filepath_200)

    csv_filepath_600 = DATA_DIR / "output_600.csv"
    ...
    logger.debug("Número de nodos en la comunidad más grande: %s", len(largest_community))
    logger.debug("Shortest path: %s", shortest_path)
    logger.debug("Diameter: %s", diameter)

    # Para escribir en el archivo results
    #csv_filepath2 = DATA_DIR / "results.csv"
